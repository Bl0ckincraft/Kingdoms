...
import fr.blockincraft.kingdoms.listeners.MenuListener;
...
public abstract class ChestMenu {
    ...
    public ChestMenu(String title, int lines) {
        ...
        for (int i = 0; i < 9 * lines; i++) {
            this.addItem(i, Parameters.defaultMenuItem, (p, slot, item, click) -> false);
        }
    }
    ...
    public void open(Player... players) {
        ...
        for (Player p : players) {
            ...
            MenuData.menus.put(p.getUniqueId(), this);
            if (open != null)
                open.onOpen(p);
        }
    }
    ...
    public interface MenuClickHandler {
        ...
        public boolean onClick(Player p, int slot, ItemStack item, ClickType click);
    }
